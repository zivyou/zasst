"""an agent to search & match job for myself"""
import json
import re
import sqlite3
...
def search_jobs_node(state: JobSearchState) -> JobSearchState:
    ...
    for result in search_result.get("results", []):
        candidates.append({
            ...
            "publish_date": extract_date(result.get("content", default=""))
        })
...
def match_job_node(state: JobSearchState) -> JobSearchState:
    ...
    # 查询简历信息
    try:
        resume_info = resume.query("我的技术栈、工作经验和技能")
    except IOError as e:
        print(f"⚠️  简历查询失败: {e}")
...
def check_completion_node(state: JobSearchState) -> Dict[str, Any]:
    ...
    if len(matched_jobs) >= target_count:
        ...
        with open("./data/job_candidates.json", "w", encoding='utf-8') as f:
